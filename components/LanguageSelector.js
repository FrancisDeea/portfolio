...
export default function LanguageSelector({ lang, font }) {
    ...
    return (
        <div className={styles.container} style={isActive ? { "width": "280px" } : { "transitionDelay": "100ms" }} onClick={() => setIsActive(false)}>
            <button className={`${styles.btn_lang}`} onClick={(e) => { setIsActive(true); e.stopPropagation(); }} style={isActive ? { "opacity": "0" } : { "opacity": "1", "transitionDelay": "800ms" }}>{lang["btn-lang"]}</button>
